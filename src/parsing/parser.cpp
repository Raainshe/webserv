...
#include "../../includes/structs/location_config.hpp"
#include "../../includes/structs/main_config.hpp"
#include "../../includes/structs/server_config.hpp"
...
void expect(TokenStream &ts, TokenType type, const std::string &msg) {
  if (ts.peek().type != type)
    throw std::runtime_error("Parse error: expected " + msg + ", got '" +
                             ts.peek().value + "'");
}
...
size_t parseSizeWithSuffix(const std::string &str) {
  ...
  if (*end == 'K' || *end == 'k')
    ...
    throw std::runtime_error("Parse error: invalid size suffix in '" + str +
                             "'");
  if (val < 0)
    throw std::runtime_error("Parse error: negative size value");
...
bool isValidErrorCode(int code) { return code >= 400 && code <= 599; }

void validateHttpMethods(const std::vector<std::string> &methods) {
  std::set<std::string> seen;
  for (size_t i = 0; i < methods.size(); ++i) {
    if (!isValidHttpMethod(methods[i])) {
      throw std::runtime_error("Parse error: invalid HTTP method '" +
                               methods[i] + "'");
    }
    if (seen.count(methods[i])) {
      throw std::runtime_error("Parse error: duplicate HTTP method '" +
                               methods[i] + "'");
    }
...
LocationConfig parseLocation(TokenStream &ts) {
  ...
  bool seen_root = false, seen_autoindex = false, seen_upload_store = false,
       seen_cgi_pass = false;
      ...
      if (directive == "root") {
        if (seen_root)
          throw std::runtime_error(
              "Duplicate 'root' directive in location block");
        seen_root = true;
        ...
        if (seen_directives.count("index"))
          throw std::runtime_error(
              "Duplicate 'index' directive in location block");
        ...
        if (seen_autoindex)
          throw std::runtime_error(
              "Duplicate 'autoindex' directive in location block");
        seen_autoindex = true;
        ...
        if (isTrue(val))
          ...
          throw std::runtime_error(
              "Parse error: invalid value for autoindex: '" + val + "'");
        expect(ts, TOKEN_SEMICOLON, "; after autoindex");
        ts.next();
      } else if (directive == "allow_methods") {
        if (seen_directives.count("allow_methods"))
          throw std::runtime_error(
              "Duplicate 'allow_methods' directive in location block");
        ...
        if (seen_upload_store)
          throw std::runtime_error(
              "Duplicate 'upload_store' directive in location block");
        ...
        if (seen_cgi_pass)
          throw std::runtime_error(
              "Duplicate 'cgi_pass' directive in location block");
        seen_cgi_pass = true;
        ...
        // Syntax: return <3xx> <url>;
        expect(ts, TOKEN_WORD, "return status code");
        int code = std::atoi(ts.next().value.c_str());
        if (code < 300 || code > 399) {
          throw std::runtime_error(
              "Parse error: redirect code must be 3xx in 'return' directive");
        }
        expect(ts, TOKEN_WORD, "return URL");
        std::string url = ts.next().value;
        loc.return_code = code;
        loc.return_url = url;
        expect(ts, TOKEN_SEMICOLON, "; after return");
        ts.next();
      } else {
        while (ts.peek().type != TOKEN_SEMICOLON &&
               ts.peek().type != TOKEN_RBRACE && !ts.eof())
          ts.next();
...
ServerConfig parseServer(TokenStream &ts) {
  ServerConfig srv;
  bool seen_listen = false, seen_server_name = false,
       seen_client_max_body_size = false;
  std::set<std::string> seen_directives;
  ...
  while (ts.peek().type != TOKEN_RBRACE && !ts.eof()) {
    if (ts.peek().type == TOKEN_WORD && ts.peek().value == "location") {
      ...
      if (directive == "listen") {
        if (seen_listen)
          throw std::runtime_error(
              "Duplicate 'listen' directive in server block");
        seen_listen = true;
        expect(ts, TOKEN_WORD, "listen value");
        int port = std::atoi(ts.next().value.c_str());
        if (!isValidPort(port)) {
          throw std::runtime_error("Parse error: invalid port number '" +
                                   ts.peek().value + "' (must be 1-65535)");
        }
        ...
        if (seen_server_name)
          throw std::runtime_error(
              "Duplicate 'server_name' directive in server block");
        ...
        if (seen_client_max_body_size)
          throw std::runtime_error(
              "Duplicate 'client_max_body_size' directive in server block");
  ...
  if (!seen_listen)
    throw std::runtime_error(
        "Missing required 'listen' directive in server block");
  return srv;
}
} // namespace
...
MainConfig parseConfig(const std::vector<Token> &tokens) {
  ...
  while (!ts.eof()) {
    if (ts.peek().type == TOKEN_WORD && ts.peek().value == "server") {
      ...
      throw std::runtime_error(
          "Parse error: expected 'server' block at top level, got '" +
          ts.peek().value + "'");
    }
  }
  return config;
